...
const validate = ({ required }) => (req, res, next) => {
    ...
    if (required && (!email || !email.trim() || !password || !password.trim() || !password || !password.trim())) {
...
const create = (req, res, next) => {
    const _a = req.body, { email, password, login_as, noHandphone, name} = _a, rest = __rest(_a, ["email", "password"]);
    const { db } = req.app;
    if (db == null) {
...
=======
    const { email, password } = req.body;
>>>>>>> eb8797786d4aedd31a83fb09ba3e36ae78e17893
    const { db } = req.app;
    if (db == null) {
        throw Error('You must bind the router db to the app');
    }
...
<<<<<<< HEAD
    const role = db.get('role').find({login_as}).value()
=======
>>>>>>> eb8797786d4aedd31a83fb09ba3e36ae78e17893
    if (!user) {
